...
import {
  CalendarClock, ChevronDown, ChevronLeft, ChevronRight, Clock3,
  FileUp, LogOut, Mail, Paperclip, Send, SendHorizontal, Settings,
  Star, Upload, Undo2, Redo2, Bold, Italic, Underline, AlignLeft,
  ListOrdered, List, IndentDecrease, IndentIncrease, Quote, Link2,
  Strikethrough, Search, X, Check, UserCircle2
} from "lucide-react";
import {
  Email, Sender, User, getEmails, getMe, getSenders, loginWithGoogle,
  logout, scheduleEmail, connectSlack, disconnectSlack, getSlackChannels,
  getSlackStatus, setSlackChannel, type SlackChannel, type SlackStatus
} from "./api";

type View = "scheduled" | "sent" | "compose" | "detail" | "settings";
type ScheduleChoice = "tomorrow" | "10am" | "11am" | "3pm" | null;

function App() {
  const [user, setUser] = useState<User | null>(null);
  const [loading, setLoading] = useState(true);
  const [view, setView] = useState<View>(location.pathname === "/settings/slack" ? "settings" : "scheduled");
  ...
  const scheduled = emails.filter((e) =>
    ["scheduled", "queued", "processing", "sending", "rate_limited"].includes(e.status)
  );
  ...
  return (
    <div className="app-shell">
      <Sidebar
        ...
        onNavigate={(next) => { setSelected(null); setView(next); }}
      ...
      <main className="main">
        {view === "compose" && (
          <Composer
            user={user}
            senders={senders}
            onDone={async () => { await load(); setView("scheduled"); }}
          />
        )}
      ...
      <div className="top-user">
        ...
        {profileOpen && (
          <div className="profile-menu">
            ...
            <button onClick={async () => { await logout(); location.reload(); }}>
  ...
  return (
    <div className="login-screen">
      <div className="login-card">
        ...
        <div className="or"><span />or sign up through email<span /></div>
...
function Sidebar({
  user, view, scheduledCount, sentCount, onCompose, onNavigate, onSettings,
}: {
  user: User;
  view: View;
  ...
  return (
    <aside className="sidebar">
      ...
      <div className="sidebar-bottom">
        <button className={`muted-nav ${view === "settings" ? "active" : ""}`} onClick={onSettings}><Settings size={16} /> Settings</button>
      </div>
    </aside>
  );
}
...
function SlackSettings() {
  ...
  if (loading) {
    return <section className="settings-page"><div className="settings-card"><div className="loader" /></div></section>;
  }
        ...
        {!status?.connected ? (
          ...
          <>
            ...
            <div className="slack-channel-field">
              ...
              <small>
                Only channels where the ReachInbox Slack app is a member are shown.
              </small>
            </div>
            ...
            <div className="settings-actions">
              <button className="slack-save-button" disabled={!selectedChannel || saving} onClick={() => void save()}>
                {saving ? "Saving..." : "Save channel"}
              </button>
              <button className="slack-disconnect-button" disabled={disconnecting} onClick={() => void disconnect()}>
...
function Mailbox({
  title, emails, scheduled, onSelect,
}: {
  title: string;
  emails: Email[];
  scheduled?: boolean;
      ...
      <div className="message-list">
        {emails.length === 0 ? (
          <div className="empty-state">
            <Mail size={28} />
            <strong>No {title.toLowerCase()} emails</strong>
            <span>Compose an email to see it here.</span>
          </div>
        ) : emails.map((email) => (
          <button className="message-row" key={email.id} onClick={() => onSelect(email)}>
            <div className="row-to">To: {email.to_email}</div>
            <div className="row-main">
              {scheduled ? (
                <span className="scheduled-pill"><Clock3 size={12} /> {formatScheduled(email.scheduled_at)}</span>
              ) : (
                <span className="sent-pill">Sent</span>
              )}
              <strong>{email.subject || "(No subject)"}</strong>
              <span className="row-preview">- {email.text_body.replace(/\n/g, " ").slice(0, 110)}</span>
            </div>
  ...
  return (
    <section className="detail">
      <div className="detail-top">
        <button className="back-button" onClick={onBack}><ChevronLeft size={17} /> Back</button>
        <span>{email.sent_at ? new Date(email.sent_at).toLocaleString() : formatScheduled(email.scheduled_at)}</span>
      </div>
      ...
      <div className="sender-line">
        ...
        <span className="detail-date">{email.sent_at ? new Date(email.sent_at).toLocaleString() : ""}</span>
      </div>
      ...
      {email.preview_url && (
        <a className="preview-link" href={email.preview_url} target="_blank" rel="noreferrer">
  ...
  return (
    <section className="composer">
      <div className="compose-header">
        <h2>New Email</h2>
        <button className="close-compose" onClick={onDone}><X size={19} /></button>
      </div>
        ...
        <div className="recipient-wrap">
          {recipients.map((recipient) => (
            <span className="recipient-chip" key={recipient}>
              {recipient}<button onClick={() => setRecipients((r) => r.filter((x) => x !== recipient))}><X size={12} /></button>
            </span>
          ))}
          <input
      ...
      <div className="limits-row">
        ...
        <input value={delay} onChange={(e) => setDelay(e.target.value.replace(/\D/g, "").slice(0, 3))} placeholder="00" />
        <span>Hourly Limit</span>
        <input value={hourlyLimit} onChange={(e) => setHourlyLimit(e.target.value.replace(/\D/g, "").slice(0, 3))} placeholder="00" />
      </div>
      ...
      <div className="editor">
        ...
        <div className="editor-toolbar">
          <button><Undo2 size={17} /></button><button><Redo2 size={17} /></button>
          <span />
          <button> T<small>↕</small></button><button><Bold size={17} /></button>
          <button><Italic size={17} /></button><button><Underline size={17} /></button>
          <span />
          <button><AlignLeft size={17} /></button><button>↕</button>
          <button><ListOrdered size={17} /></button><button><List size={17} /></button>
          <button><IndentIncrease size={17} /></button><button><IndentDecrease size={17} /></button>
          <button><Quote size={17} /></button><button><Link2 size={17} /></button>
          <button><Strikethrough size={17} /></button>
        </div>
      </div>

      <div className="compose-actions">
        <button className="attach"><Paperclip size={18} /></button>
        <div className="schedule-area">
          <button className="send-later" onClick={() => setScheduleOpen((v) => !v)}>
          ...
          {scheduleOpen && (
            <div className="schedule-popover">
              ...
              <input
                type="datetime-local"
                value={dateValue}
                onChange={(e) => { setDateValue(e.target.value); setChoice(null); }}
              />
              ...
              <div className="popover-actions">
                ...
                <button className="done" onClick={() => setScheduleOpen(false)}><Check size={15} /> Done</button>
              </div>
            </div>
          )}
        </div>
