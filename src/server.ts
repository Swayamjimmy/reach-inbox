 ...
 * A temporary Elasticsearch failure must not prevent the API
 * from starting or the existing PostgreSQL/BullMQ workflow
 * from working.
 */
void syncEmailSearchIndex(async () => {
  ...
  console.error(
    "Elasticsearch startup sync failed:",
    error,
  );
});
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      res.redirect(
        `${config.frontendOrigin}/settings/slack`,
      );
    } catch (error) {
      const message =
      ...
      res.redirect(
        `${config.frontendOrigin}/login?error=${encodeURIComponent(
          message,
        )}`,
      );
    }
...
app.post(
  ...
  async (req, res) => {
    ...
    const senderId = requiredString(
      req.body.senderId,
      "senderId",
    );

    const toEmail = requiredString(
      req.body.toEmail,
      "toEmail",
    );
    ...
    try {
      ...
      res.status(503).json({
        error:
          "Email was saved but Redis enqueue failed; startup recovery will retry",
        emailId: email.id,
      });
    ...
    /*
     * Indexing is deliberately best-effort.
    ...
    if (saved.rows[0]) {
      try {
        ...
        console.error(
          "Elasticsearch indexing failed after scheduling:",
          error,
        );
      }
    }
 ...
 * - sender ID
 *
 * Search results are always restricted to the
...
const server = app.listen(
  ...
  () => {
    ...
    console.log(
      `Recovered ${recovered} non-final email records`,
    );
  },
);
